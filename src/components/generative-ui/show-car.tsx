...
const CarImage = ({ car }: { car: Car }) => {
  const imageSrc = car?.image?.src || "/images/default-car.svg";
  const imageAlt = car?.image?.alt || `${car.year} ${car.make} ${car.model}`;
  
  return (
    <div className="relative aspect-[3/3] w-full overflow-hidden h-[250px] bg-gray-100">
      <Image
        width={300}
        height={250}
        src={imageSrc}
        ...
        onError={(e) => {
          ...
          (e.target as HTMLImageElement).src = "/images/default-car.svg";
        }}
      />
    </div>
  );
...
export function ShowCar({ car, onSelect, onReject, status, className }: ShowCarProps) {
  // Safety check - return early if car is undefined or missing required fields
  if (!car || !car.make || !car.model || !car.year || car.price === undefined) {
    return (
      <div className="p-4 text-gray-500 bg-gray-50 rounded-lg">
        <p>Vehicle information unavailable</p>
        <p className="text-sm">Please try selecting another vehicle.</p>
      </div>
    );
  }
...
export function ShowCars({ cars, onSelect, status }: ShowCarsProps) {
  ...
  // Safety check - return early if cars is undefined or not an array
  if (!cars || !Array.isArray(cars)) {
    return (
      <div className="p-4 text-gray-500">
        No vehicles available at the moment. Please try again.
      </div>
    );
  }
